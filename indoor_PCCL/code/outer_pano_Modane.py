...
if __name__ == "__main__":
    ...
    parser.add_argument("--entry_json", help="path for debugging perpusses to read input from json")
    ...
    pccl_utils.time_from(t1, "LOADED PLY AND CALCULATED ITS BBOX", logger, args.debug)
    print()
    ...
    if not args.queries_glob:
        ...
        pccl_utils.time_from(t3, "CONNECTED TO CAMERA", logger, args.debug)
    ...
    if args.broker_ip:
        ...
        #PublishingTopic = 'fromtool-loc-ibl'
        import paho.mqtt.client as mqtt
        client = mqtt.Client(ToolId)
        client.connect(args.broker_ip)
        # Retrieve FRID
        msg = subscribe.simple('config-info', qos=0, retained=False, hostname=args.broker_ip)
        myGlobalMessagePayload = msg.payload.decode()
        frID = json.loads(myGlobalMessagePayload)['frID']
    pccl_utils.time_from(t4, "CONNECTED TO BROKER", logger, args.debug)
    print()
    ...
    while frame_count <= n_frames:
        ...
        if not args.queries_glob:
            ...
            pccl_utils.time_from(t5, "CAPTURED AND DOWNLOADED IMAGE", logger, args.debug)
        else:
            inner_args.query_img = queries[frame_count]

        # Read from Inertio
        t55 = time.time()
        if inner_args.entry_json:
            with open(inner_args.entry_json) as user_file:
                data = json.loads(user_file.read())['infoprioPayload']['toolPayload'][0]['toolData'][0]
        else:
            while(True):
                msg = subscribe.simple('fromtool-loc-ibl', qos=0, retained=False, hostname=inner_args.broker_ip)
                myGlobalMessagePayload = msg.payload.decode()
                if json.loads(myGlobalMessagePayload)['infoprioPayload']['toolPayload'][0]['source']['frID'] == frID:
                    data = json.loads(myGlobalMessagePayload)['infoprioPayload']['toolPayload'][0]['toolData'][0]
                    break
        dx , dz = pccl_utils.distance_between_global_coordinates(inner_args.model_lat, inner_args.model_lon, data['latitude'], data['longitude'], inner_args.model_north)
        dy = (room_bbox['y_min'] + inner_args.min_dist_from_ceiling_m + room_bbox['y_max'] - inner_args.min_dist_from_floor_m) /2
        init_point_xyz = np.array([dx, dy, dz])
        pccl_utils.time_from(t55, "TOOK INPUT FROM INERTIO", logger, args.debug)
        
        # Calculate initial positions sub-set
        init_poses_subset = pccl_utils.initial_poses_from_inertio(init_point_xyz, room_bbox, inner_args, device)
        ...
        pccl_utils.time_from(t6, "LOCALIZED IMAGE", logger, args.debug)
        ...
        pccl_utils.time_from(t0, "*** LOOP " + str(frame_count) + " TOTAL TIME", logger, args.debug)
        print()
        frame_count += 1
