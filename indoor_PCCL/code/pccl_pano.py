...
def pccl_pano(args_inner, logger, room_xyz, room_rgb, room_bbox, init_Poses, mask_pixel_h=0):
    # Open and prepare query image
    t2 = time.time()
    device = torch.device("cpu") if not torch.cuda.is_available() else torch.device(args_inner.torch_device)
    query_img = cv2.cvtColor(cv2.imread(args_inner.query_img, cv2.IMREAD_COLOR), cv2.COLOR_BGR2RGB)
    
    # Apply white noise mask
    query_img[query_img.shape[0]-mask_pixel_h : query_img.shape[0], :, :] = \
        (np.random.rand(mask_pixel_h, query_img.shape[1], 3) * 256).astype(int)
    ...
    pccl_utils.time_from(t2, "OPEN AND PREPARE QUERY IMG", logger, args_inner.debug)
    
    # Apply image filter
    if args_inner.image_filter:
        query_img = pccl_utils.filter_image(room_rgb, query_img)
    
    if args_inner.show_save_query:
        cv2.imshow("--show_save_query", cv2.cvtColor(query_img, cv2.COLOR_RGB2BGR))
        cv2.waitKey(0)
        cv2.imwrite(os.path.join(args_inner.output_dir, "query.png"), cv2.cvtColor(query_img, cv2.COLOR_RGB2BGR))
    ...
    if not args_inner.skip_histo:
        ...
        for pose in init_Poses:
            ...
            sampled_query_img, vertex_dist_from_camera = pccl_utils.sample_query_img_pano(extrinsics, query_img, width, height, room_xyz, device)
            hist_loss.append(pccl_utils.histogram_loss_pano(room_rgb, sampled_query_img, args_inner))
            if args_inner.show_projections: pccl_utils.show_or_save_pano(extrinsics, width, height, room_xyz, room_rgb, device, args_inner.dilate_outs, name="--show_projections")   
        ...
        if args_inner.show_best_histo:
            ...
            for i, m, n in zip(sorted_indices, sorted_hist_loss, init_Poses):
                if args_inner.query_img_poses:
                    metric = pccl_utils.calculate_metrics_pano(n, args_inner, device)
                ...
                pccl_utils.show_or_save_pano(pccl_utils.calculate_extrinsics(n, device), width, height, room_xyz, room_rgb, device, args_inner.dilate_outs, name="--show_best_histo")
    ...
    for i in range(len(init_Poses)):
        ...
        for j in range(args_inner.itters):
            ...
            extrinsics = pccl_utils.calculate_extrinsics(pose, device)
            sampled_query_img, vertex_dist_from_camera = pccl_utils.sample_query_img_pano(extrinsics, query_img, width, height, room_xyz, device)
            loss = pccl_utils.sampling_loss_pano(
                room_rgb, sampled_query_img, pose, room_bbox, vertex_dist_from_camera, args_inner, args_inner.sampl_loss, args_inner.penalty_factor, args_inner.distance_w_lamda)
            loss.backward()
            ...
            if args_inner.show_optimization:
                ...
                pccl_utils.show_or_save_pano(extrinsics, width, height, room_xyz, room_rgb, device, args_inner.dilate_outs, name="--show_optimization", wait=1)
    ...
    logger.info(f"\nBest pose: {best_pose.cpu().detach().numpy()}     Loss: {best_loss.item()}")
    if args_inner.query_img_poses:
        # best_metric = calculate_metrics(best_pose, args_inner, device)
        best_metric = pccl_utils.calculate_metrics_from_name(best_pose, args_inner, device)
    ...
    if args_inner.save_best:
        ...
        pccl_utils.show_or_save_pano(pccl_utils.calculate_extrinsics(best_pose, device), width, height, room_xyz, room_rgb, device, args_inner.dilate_outs,
            path = f_base + ".png")
...
if __name__ == "__main__":
    ...
    # Localization
    dx, dy, dz, dyaw, dpitch, droll = pccl_pano(args, logger, room_xyz, room_rgb, room_bbox, init_Poses, args.mask_pixel_h)
