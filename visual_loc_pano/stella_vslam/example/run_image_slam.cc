...
int mono_tracking(const std::shared_ptr<stella_vslam::system>& slam,
		  ...
		  const std::string lockfile_md5hash) {
    // load the mask image
    ...
    pangolin_viewer::viewer viewer(
        stella_vslam::util::yaml_optional_ref(cfg->yaml_node_, "PangolinViewer"), slam, slam->get_frame_publisher(), slam->get_map_publisher(), false, socket_file, lockfile_md5hash);
#elif USE_SOCKET_PUBLISHER
