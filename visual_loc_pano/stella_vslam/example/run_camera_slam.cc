...
void captureFunc()
{
    ...
    params.push_back(cv::CAP_PROP_N_THREADS);
    params.push_back(4);

    ret = sprintf(connstr,"udp://%s:%d?overrun_nonfatal=1&fifo_size=500000", c_hostname.c_str(), c_port);
    //ret = sprintf(connstr,"udp://%s:%d?overrun_nonfatal=1", c_hostname.c_str(), c_port);
 
    cap = new VideoCapture(std::string(connstr), cv::CAP_FFMPEG, params);
    //cap = new VideoCapture(std::string(connstr));
...
int equirectangular_tracking(const std::shared_ptr<stella_vslam::system>& slam,
    ...
    pangolin_viewer::viewer viewer(
        stella_vslam::util::yaml_optional_ref(cfg->yaml_node_, "PangolinViewer"), slam, slam->get_frame_publisher(), slam->get_map_publisher(), enable_debug, socket_file, md5hash_lockfile);
#elif USE_SOCKET_PUBLISHER
    ...
    //spdlog::info("joining capThread");
    //capThread.join();
    //spdlog::info("capThread joined!");

    //spdlog::info("joining keepAliveThread");
    //keepAliveThread.join();
    //spdlog::info("keepAliveThread joined!");
...
int mono_tracking_srv(const std::shared_ptr<stella_vslam::system>& slam,
                  const std::shared_ptr<stella_vslam::config>& cfg,
                  const unsigned int cam_num,
                  const std::string& mask_img_path,
                  const float scale,
		  ...
		  const int server_udp_port,
		  const int runs_in_vscode
		  ) {
...
#ifdef USE_PANGOLIN_VIEWER
    pangolin_viewer::viewer viewer(
        stella_vslam::util::yaml_optional_ref(cfg->yaml_node_, "PangolinViewer"), slam, slam->get_frame_publisher(), slam->get_map_publisher(), disable_gui, socket_file, md5hash_lockfile);
#elif USE_SOCKET_PUBLISHER
    socket_publisher::publisher publisher(
        stella_vslam::util::yaml_optional_ref(cfg->yaml_node_, "SocketPublisher"), slam, slam->get_frame_publisher(), slam->get_map_publisher());
#endif
    ...
    c_hostname = server_ip;
    c_port = server_udp_port;
    ...
    std::thread thread([&]() {
        while (true) {
            ...
            if (frame.empty()) {
	        spdlog::info("Skipping empty frame in mono_tracking_srv.");
                continue;
            }

            cv::resize(frame, frame, cv::Size(640,360), cv::INTER_LANCZOS4);
...
void lockfile_cleanup_func(std::string fpath_) {
    spdlog::info("Calling pre-run clean up hook.");

    if(access(fpath_.c_str(), F_OK) == 0) {
	spdlog::info("removing lockfile at path {}.", fpath_);
        remove(fpath_.c_str());
    }
}
...
int main(int argc, char* argv[]) {
    ...
    auto use_gpmax = op.add<popl::Implicit<bool>>("M", "usegopromax", "connect to a GoPro Max camera via WiFi", false);
    auto use_gphero10 = op.add<popl::Implicit<bool>>("W", "usegoprohero10", "connect a GoPro HERO 10 camera via cable", true);
    ...
    auto socket_file = op.add<popl::Value<std::string>>("S","unix-socket path", "path to Unix-domain socket", "tpf_unix_sock.server");
    auto runs_in_vscode = op.add<popl::Value<bool>>("V", "runs-in-vscode", "define whether run_camera_slam runs from a VS Code instance", false);
    ...
    if((use_gpmax->is_set() && use_gphero10->is_set()) || (!use_gpmax->is_set() && !use_gphero10->is_set())) {
        std::cerr << "Invalid choice of a GoPro camera model. Need to set either 'max' or 'hero'." << std::endl;
	return EXIT_FAILURE;
    }
    ...
    lockfile_cleanup_func("/tmp/.LOCK_FILE");
    ...
    else if (use_gphero10->value() == true && \
	       use_gpmax->value() == false && \
	         slam->get_camera()->setup_type_ == stella_vslam::camera::setup_type_t::Monocular)
    {
        ret = mono_tracking_srv(slam,
                                cfg,
                                cam_num->value(),
                                mask_img_path->value(),
                                scale->value(),
